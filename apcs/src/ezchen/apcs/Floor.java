...
public class Floor {
	...
	private void makeWalls() {
		for (int r = 0; r < tiles.length; r++) {
			setTile(r, (int) position.x, new Tile(true, new Vector2(position.x, position.y - r)));
			
			int c = (int) position.x + tiles[r].length - 1;
			setTile(r, c, new Tile(true, new Vector2(position.x + c, position.y - r)));
		}
	}
	...
	private void makeFloor() {
		...
		for (; numHoles > 0; numHoles--) {
			int holePosition = (int) (1 + ((WIDTH - 2) * Math.random()));
